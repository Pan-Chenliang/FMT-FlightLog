...
FMT FlightLog 是一个面向 FMT MLog 日志的纯前端解析工具。它可以在浏览器本地读取 `.bin` / `.log` / `.mlog` 等 MLog 格式日志，解析消息定义、参数组和数据帧，自动生成摘要、消息表、快速曲线，并导出 JSON / CSV。
...
- **自动绘图**：自动选择有变化的数值字段，最多展示 6 张快速曲线。
- **数据导出**：支持下载日志摘要 JSON，以及按消息类型导出的 CSV。
- **缓存恢复**：使用 IndexedDB 缓存最近一次上传的日志，刷新页面后可自动恢复。
- **飞控导入**：在支持 Web Serial 的浏览器中，可通过 MAVLink FTP 从飞控读取日志。
- **静态部署**：无需后端、数据库或构建步骤，适合 GitHub Pages 托管。
...
3. 等待页面解析完成，查看状态栏、日志摘要、消息表、参数组和快速图表。
4. 在“快速图表”区域下载摘要 JSON 或某个消息类型的 CSV。
5. 如需清理浏览器缓存，点击“清除缓存”。
